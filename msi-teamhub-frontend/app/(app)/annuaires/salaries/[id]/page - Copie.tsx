...
const [
  ...
  fetch(`http://localhost:8000/api/creneaux-travail/`, { headers }),
        ...
        const deptArray = Array.isArray(departementsData) 
        ? departementsData 
        : (departementsData.results || []);
        console.log('Departments set to:', deptArray);
        setDepartements(deptArray);
...
const getDepartementInfo = (deptId: number) =>
  ...
  useEffect(() => {
  if (salarie?.departements && departements.length > 0) {
    console.log('🔍 Salarie IDs:', salarie.departements);
    console.log('🔍 All dept IDs in DB:', departements.map(d => d.id).slice(0, 20));
    console.log('🔍 Max dept ID:', Math.max(...departements.map(d => d.id)));
    console.log('🔍 Total depts loaded:', departements.length);
  }
}, [salarie, departements]);

// ✅ AJOUTE LE NOUVEAU ICI
useEffect(() => {
  if (salarie?.responsable_direct) {
    console.log('🔍 Salarie responsable_direct ID:', salarie.responsable_direct);
    console.log('🔍 All salaries IDs:', salaries.map(s => s.id).slice(0, 20));
    console.log('🔍 Max salaries ID:', Math.max(...salaries.map(s => s.id)));
    console.log('🔍 Total salaries loaded:', salaries.length);
    
    const found = salaries.find(s => s.id === salarie.responsable_direct);
    console.log('🔍 Responsable found?:', found ? '✅ YES' : '❌ NO');
  }
}, [salarie, salaries]);
