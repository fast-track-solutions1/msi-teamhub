...
import { X, Loader2, AlertCircle, Search, ChevronDown, Clock, RotateCcw } from 'lucide-react';
...
export default function SalarieForm({
  ...
  const [formData, setFormData] = useState({
    ...
    // ✅ NOUVEAU - Horaires de pause personnalisables
  ...
  // ✅ NOUVEAU - État pour tracker les modifications de pause
  const [pauseModified, setPauseModified] = useState(false);
  ...
  // ✅ NOUVEAU - Récupérer les infos du créneau et remplir les horaires de pause
  useEffect(() => {
    if (formData.creneau_travail) {
      ...
      // Si c'est une nouvelle sélection ET qu'il n'y a pas de pause personnalisée
      if (creneau && !pauseModified) {
        setFormData((prev) => ({
          ...prev,
          heure_pause_debut: creneau.heure_pause_debut,
          heure_pause_fin: creneau.heure_pause_fin,
        }));
      }
    } else {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (type === 'checkbox') {
      ...
      // ✅ NOUVEAU - Tracker si la pause a été modifiée
      if (['heure_pause_debut', 'heure_pause_fin'].includes(name)) {
        setPauseModified(true);
      }
    }
  ...
  const handleCreneauRemove = () => {
    ...
    setPauseModified(false);
  };

  // ✅ NOUVEAU - Réinitialiser les horaires de pause du créneau
  const handleResetPause = () => {
    if (selectedCreneauDetails) {
      setFormData((prev) => ({
        ...prev,
        heure_pause_debut: selectedCreneauDetails.heure_pause_debut,
        heure_pause_fin: selectedCreneauDetails.heure_pause_fin,
      }));
      setPauseModified(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('📤 Envoi du formulaire avec les données :', {
      ...formData,
      societe_valeur: formData.societe,
      departements_valeur: formData.departements,
      responsable_direct_valeur: formData.responsable_direct,
      creneau_travail_valeur: formData.creneau_travail,
      pause_modifiee: pauseModified,
    });
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* ✅ NOUVEAU - Afficher les infos du créneau et champs modifiables pour pause */}
            {selectedCreneauDetails && (
              <div className="md:col-span-2 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4">
                <div className="flex items-start justify-between gap-3 mb-4">
                  <h4 className="text-sm font-bold text-purple-900 dark:text-purple-300 flex items-center gap-2">
                    <Clock size={18} />
                    Horaires du créneau "{selectedCreneauDetails.nom}"
                  </h4>
                  {pauseModified && (
                    <button
                      type="button"
                      onClick={handleResetPause}
                      title="Réinitialiser à l'horaire du créneau"
                      className="px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 rounded text-xs font-medium hover:bg-blue-200 dark:hover:bg-blue-800 transition flex items-center gap-1"
                    >
                      <RotateCcw size={14} />
                      Réinitialiser
                    </button>
                ...
                {/* Grille affichage infos + champs modifiables */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {/* Heure début (READ-ONLY) */}
                  ...
                  {/* ✅ Pause début (MODIFIABLE) */}
                  <div className="bg-white dark:bg-slate-800 rounded p-2 border border-orange-300 dark:border-orange-700">
                    ...
                    <input
                      ...
                      value={formData.heure_pause_debut}
                      onChange={handleChange}
                      className="w-full px-2 py-1 border border-orange-300 dark:border-orange-700 rounded bg-orange-50 dark:bg-orange-900/20 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm font-bold"
                    />
                    {pauseModified && formData.heure_pause_debut !== selectedCreneauDetails.heure_pause_debut && (
                      <div className="text-xs text-orange-600 dark:text-orange-400 mt-1">
                        <span className="font-medium">Modifié</span> {selectedCreneauDetails.heure_pause_debut}
                      </div>
                    )}
                  </div>

                  {/* ✅ Pause fin (MODIFIABLE) */}
                  <div className="bg-white dark:bg-slate-800 rounded p-2 border border-orange-300 dark:border-orange-700">
                    <label className="text-xs font-medium text-slate-600 dark:text-slate-400 block mb-1">
                      Pause fin
                    </label>
                    <input
                      ...
                      value={formData.heure_pause_fin}
                      onChange={handleChange}
                      className="w-full px-2 py-1 border border-orange-300 dark:border-orange-700 rounded bg-orange-50 dark:bg-orange-900/20 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm font-bold"
                    />
                    {pauseModified && formData.heure_pause_fin !== selectedCreneauDetails.heure_pause_fin && (
                      <div className="text-xs text-orange-600 dark:text-orange-400 mt-1">
                        <span className="font-medium">Modifié</span> {selectedCreneauDetails.heure_pause_fin}
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* Info résumé */}
                <div className="mt-3 p-2 bg-purple-100 dark:bg-purple-900/30 border border-purple-300 dark:border-purple-700 rounded text-xs text-purple-900 dark:text-purple-300">
                  <strong>Travail:</strong> {selectedCreneauDetails.heure_debut} → {selectedCreneauDetails.heure_fin}
                  <br />
                  <strong>Pause:</strong> {formData.heure_pause_debut} → {formData.heure_pause_fin}
                  {pauseModified && (
                    <span className="ml-2 text-orange-600 dark:text-orange-400">
                      ⚠️ Pause personnalisée
                    </span>
                  )}
                </div>
              </div>
