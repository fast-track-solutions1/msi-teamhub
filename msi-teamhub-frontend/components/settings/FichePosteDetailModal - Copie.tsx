...
const handleGeneratePDF = () => {
  const titreComplet = `${gradeNom}${serviceNom !== 'Non renseigné' ? ` - ${serviceNom}` : ''}`;
    ...
    generateFichePDF({
      ...
      createdInfo,
      lastModInfo,
      serviceNom: serviceNom !== 'Non renseigné' ? serviceNom : '',
      gradeNom: gradeNom !== 'Non renseigné' ? gradeNom : '',
      echelonGrade,
      statut: getStatutLabel(fiche.statut),
      titulaireNom: fiche.responsable_info || '',
      descriptionPoste: fiche.description_poste || fiche.description || '',
      tachesResponsabilites: fiche.taches_responsabilites || fiche.taches || '',
      competencesRequises: fiche.competences_requises || fiche.competencesrequises || '',
    });
  };
