...
# ============================================
# ✅ REMPLACE LA CLASSE FichePosteDetailSerializer COMPLÈTE

from rest_framework import serializers
from .models import FichePoste, Salarie

class FichePosteDetailSerializer(serializers.ModelSerializer):
    servicenom = serializers.CharField(source='service.nom', read_only=True)
    gradenom = serializers.CharField(source='grade.nom', read_only=True)
    responsableinfo = serializers.SerializerMethodField(read_only=True)
    
    # ✅ AJOUTER CETTE DÉCLARATION
    responsableservice = serializers.PrimaryKeyRelatedField(
        queryset=Salarie.objects.all(),
        required=False,
        allow_null=True
    )
    ...
    class Meta:
        model = FichePoste
        fields = [
            'id', 'titre', 'service', 'servicenom', 'grade', 'gradenom',
            'responsableservice', 'responsableinfo', 'description', 'taches',
            'competencesrequises', 'moyenscorrection', 'problemes',
            'propositions', 'defauts', 'statut', 'outils', 'ameliorations',
            'datecreation', 'datemodification'
        ]
        readonly_fields = ['datecreation', 'datemodification']
    
    def get_responsableinfo(self, obj):
        if obj.responsableservice:
            return f"{obj.responsableservice.prenom} {obj.responsableservice.nom}"
        return None
